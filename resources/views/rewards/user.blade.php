        ...
        <table class="table table-hover table-bordered">
            ...
            <tfoot>
                ...
                {{--<tr>
                    ...
                    <td class="linie_t">{{round(0.4*$gutscheine_aus_assignments)}}</td>
                </tr>--}}
            </tfoot>
        </table>
    </div>
...
<div class="row">
    <div class="col-md-6">
        <p>Alle Stunden: <b><span id='ttotal'>{{TimecalcController::MinToString($t_total)}}</span></b><small> (davon bereits abgeschlossen: {{TimecalcController::MinToString($t_total_confirmed)}})</small></span></p>
    </div>
</div>
...
<div class="row">
    <div class="col-md-12">
        <table class="table table-sm table-hover table-bordered">
            ...
            <th scope="col">Dauer [h]</th>
            <th scope="col" style="border-left: solid 2px #aaa!important;">Gutscheine</th>
            </thead>
            <tbody>
            @foreach($accepted as $a)
                <tr style="background-color:#dff0d8;">
                    ...
                    <td style="border-left: solid 2px #aaa!important;">{{round($a->shift->gutscheine * Carbon::parse($a->start)->diffInMinutes(Carbon::parse($a->end)) / 60,2)}}</td>
                </tr>
            @endforeach
            @foreach($confirmed as $c)
                <tr>
                    ...
                    <td style="border-left: solid 2px #aaa!important;"><span id="gc{{$c->id}}">{{round($c->shift->gutscheine * Carbon::parse($c->start)->diffInMinutes(Carbon::parse($c->end)) /60,2)}}</span></td>
                </tr>
            @endforeach
            </tbody>
            <tfoot>
            <tr style="border-top: solid 2px #aaa!important;">
                <td colspan="4" style="text-align:right;"><b>Σ Summe</b></td>
                <td>{{round($gutscheine_selected,0)}}</td>
            </tr>
            @if($abzug > 0)
            <tr>
                <td colspan="4" style="text-align:right;"><i>Abzug Bonus Solidaritätsstunden</i></td>
                <td >-{{round($abzug,2)}}</td>
            </tr>
            @endif
            <tr>
                <td colspan="4" style="text-align:right;"><i>Bereits erhalten</i></td>
                <td >-{{round($gutscheine_gesamt,2)}}</td>
            </tr>
            <tr style="border-top: solid 2px #aaa!important;">
                <td colspan="4" style="text-align:right;"><b>Offen</b></td>
                <td><b>{{round($gutscheine_selected - $abzug - $gutscheine_gesamt)}}</b></td>
            </tr>
            {{--<tr>
                <td colspan="6" style="text-align:right;">Freigegeben</td>
                <td class="linie_t">{{round(0.4*$gutscheine_aus_assignments)}}</td>
